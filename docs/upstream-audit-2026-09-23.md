...
| [#585 durable backlog visibility](https://github.com/stephenschoettler/hermes-lcm/issues/585) | Read-only post-frontier diagnostic and bounded-page primitive; compaction gap remains open | `lcm_status.store.post_frontier` and `/lcm status` show upper-bound diagnostics, not eligible debt. `MessageStore.get_session_messages_between` now supplies an indexed, bounded, exclusive-cursor page. `tests/test_store_backlog_compaction.py` is a strict expected-failure end-to-end gate; `docs/store-backed-compaction-design.md` records the source-lineage, fresh-tail, atomic-frontier, and budget invariants. The current leaf/debt gates and compaction loop still select only the assembled window; the regression remains expected to fail until the full store-backed path passes. |
