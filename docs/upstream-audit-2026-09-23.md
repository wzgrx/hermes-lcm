...
| [#585 durable backlog visibility](https://github.com/stephenschoettler/hermes-lcm/issues/585) | Read-only post-frontier store diagnostic; compaction gap remains open | `lcm_status.store.post_frontier` and `/lcm status` now reveal session-scoped row counts, positive token estimates, and missing estimates past a matching lifecycle frontier. These are upper-bound diagnostics, not eligible compaction debt. `_leaf_compaction_candidate_status`, `_refresh_raw_backlog_debt`, and `_should_run_deferred_maintenance` still use only the assembled window's middle slice. A store-frontier query alone does not make hidden rows compactable: the compaction loop also selects from `working_messages`. A complete fix must assemble a bounded, ordered store-backed candidate with source lineage and fresh-tail protection, then advance the frontier atomically. Foreground budget caps do not solve this backlog-visibility issue. |
